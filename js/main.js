...
async function loadGitHubPDFs(week) {
    const gridEl = document.getElementById(`pdf-grid-${week}`);
    const emptyEl = document.getElementById(`docs-empty-${week}`);
    const loadingEl = document.querySelector(`#week-panel-${week} .docs-loading`);
    
    // Nếu các element không tồn tại (chưa render trang progress), bỏ qua
    if (!gridEl || !emptyEl || !loadingEl) return;
    ...
    try {
        ...
        if (!response.ok) {
            // Nếu folder không tồn tại (404) hoặc không public (403)
            if (response.status === 404) {
                console.warn(`Thư mục ${folderPath} chưa có.`);
            } else {
        ...
        if (pdfFiles.length === 0) {
            ...
            // Raw url định dạng lại để download/view: https://raw.githubusercontent.com/{owner}/{repo}/{branch}/{path}
            const html = pdfFiles.map(file => {
                const rawUrl = file.download_url || `https://raw.githubusercontent.com/${GITHUB_OWNER}/${GITHUB_REPO}/${BRANCH_NAME}/${file.path}`;
                
                // Định dạng kích thước file
                let sizeText = 'Không rõ dung lượng';
                if (file.size) {
                    const kb = file.size / 1024;
                    if (kb > 1024) sizeText = (kb / 1024).toFixed(1) + ' MB';
                    else sizeText = kb.toFixed(0) + ' KB';
        ...
        loadingEl.style.display = 'none';
        emptyEl.innerHTML = `
            <div class="docs-empty-icon" style="color:#ef4444">⚠️</div>
            <p>Lỗi kết nối đến GitHub: ${error.message}</p>
        `;
        emptyEl.style.display = 'block';
    }
}
